...
class SonicEpsGreedyQNetwork(TFQNetwork):
    """
    A wrapper around a Q-network that adds epsilon-greedy
    exploration to the actions.
    ...
    def __init__(self, model, epsilon):
        super(SonicEpsGreedyQNetwork, self).__init__(model.session, model.num_actions,
                                                model.obs_vectorizer, model.name)
        self.model = model
        self.epsilon = epsilon
    ...
    def step(self, observations, states, backtrace_flag=False):
        result = self.model.step(observations, states)
        new_actions = []
        eps = float(self.epsilon)
        for action in result['actions']:
            if random.random() < eps:
                # Better sampling strategy for Sonic (bias towards moving right)
                rand = random.random()
                if not backtrace_flag:
                    if rand < 0.5: # right
