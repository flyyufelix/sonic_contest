...
class NStepPlayer(Player):
    ...
    def play(self, backtrace_flag=False):
        # Let the buffers fill up until we get actual
        # n-step transitions.
        while True:
            transes = self._play_once(backtrace_flag)
            if transes:
                return transes

    def _play_once(self, backtrace_flag):
        for trans in self.player.play(backtrace_flag):
...
class BatchedPlayer(Player):
    ...
    def play(self, backtrace_flag):
        if self._cur_states is None:
            self._setup()
        results = []
        for _ in range(self.num_timesteps):
            for i in range(self.batched_env.num_sub_batches):
                results.extend(self._step_sub_batch(i, backtrace_flag))
        return results

    def _step_sub_batch(self, sub_batch, backtrace_flag):
        #model_outs = self.model.step(self._last_obses[sub_batch], self._cur_states[sub_batch], backtrace_flag)
        ...
        outs = self.batched_env.step_wait(sub_batch=sub_batch) # Wait for all async environments to finish collecting trajectories
        end_time = time.time()
        transitions = []
        # loop through rollouts from all environments iteratively
