...
class WarpFrame(gym.ObservationWrapper):
    ...
    def observation(self, frame):
        ...
        #if self.counter > self.warmup and self.counter % self.interval == 0 and self.num_inference < self.max_inference :

        #    #print(frame.shape)
        #    cv2.imwrite('save_' + str(self.counter) + '.png', frame)
        #    img = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
        #    img = cv2.resize(img, (224, 224))
        #    img = np.expand_dims(img, axis=0)
        #    img = np.expand_dims(img, axis=-1)
        #    #for c in range(3):
        #    #    img[:, :, :, c] = img[:, :, :, c] - self.mean_pixel[c]
        #    preds = self.levelcls_model.predict(img)
        #    #print(np.argmax(preds))
        #    self.level_pred.append(np.argmax(preds))
