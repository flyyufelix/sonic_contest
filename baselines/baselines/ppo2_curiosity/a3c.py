...
class RunnerThread(threading.Thread):
    ...
    def start_runner(self, sess, summary_writer):
        ...
        self.start() # Start thread (method from Threading class)
...
def env_runner(env, policy, num_local_steps, summary_writer, render, predictor,
    ...
    last_features = policy.get_initial_features()  # reset lstm memory to random initiation
    length = 0
    rewards = 0
    values = 0
    if predictor is not None:
        ep_bonus = 0 # intrinsic reward
        life_bonus = 0

    while True:
        terminal_end = False
        # Datastructure to store rollouts
        rollout = PartialRollout(predictor is not None)

        # Rollout for num_local_steps steps
        for _ in range(num_local_steps):
            # run policy (i.e. make inference)
            fetched = policy.act(last_state, *last_features)
            # features = next state, value_ = value, action = action
...
class A3C(object):
    def __init__(self, env, task, visualise, unsupType, envWrap=False, designHead='universe', noReward=False):
        ...
        with tf.device(worker_device):
            ...
            if self.unsup:
                if 'state' in unsupType:
                    # Loss of forward model only (No inverse model)
                    self.predloss = constants['PREDICTION_LR_SCALE'] * predictor.forwardloss 
                else:
                    # Loss of forward and inverse model
            ...
            # Runner just like OpenAI baseline (execute and collect rollouts)
            ...
            if self.unsup:
                ...
                # Aggregate all A3C and ICM grads
    ...
    def process(self, sess):
        ...
        # Pull a rollout sequence from the queue
        ...
        if self.unsup: # Add inputs to ICM network
